...
[tck-testable tck-id-cdi-discovery]#Namely, any Jakarta Contexts and Dependency Injection bean that implements the `javax.mvc.engine.ViewEngine` interface MUST be considered as a possible target for processing by calling its `supports` method, discarding the engine if this method returns `false`#.
...
. Lookup all instances of `javax.mvc.engine.ViewEngine` available via Jakarta Contexts and Dependency Injection.
