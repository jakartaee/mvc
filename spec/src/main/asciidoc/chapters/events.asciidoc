...
The class `EventObserver` is a CJakarta Contexts and Dependency InjectionDI bean in application scope whose methods `onBeforeController` and `onAfterController` are called before and after a controller is called.
